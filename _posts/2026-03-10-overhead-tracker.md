...
**The web app** is a single `index.html` file — 1,800 lines of vanilla JavaScript with Leaflet for the map. No React, no bundler. You can clone the repo and open the file directly in a browser. It deploys to GitHub Pages on every push.
...
**The ESP32 display** is a Freenove FNK0103S with a 4" 480x320 touchscreen sitting on my shelf in the living room. It polls the Pi proxy directly over the local network and cycles through overhead flights every 8 seconds. No browser, no phone needed.
...
The web app has its own fallback chain too — it tries the proxy first (6-second timeout), then falls back to hitting the APIs directly. Clients never know which source answered.
...
Airline names come from the ICAO callsign prefix — `QFA` maps to Qantas, `CPA` to Cathay Pacific, `UAE` to Emirates. I built a lookup table of 46 airlines, each colour-coded by brand: Qantas red, Cathay green, Emirates gold. Aircraft type codes get translated too — `B789` becomes `B787-9 Dreamliner`, `A20N` becomes `A320neo`.
...
Node.js on a Pi 3B+, kept alive by PM2. Cloudflare Tunnel handles the public ingress — no port forwarding, no dynamic DNS. There's a dashboard at `dashboard.overheadtracker.com` that shows live stats: uptime, request count, cache hit rate, and a 24-hour peak traffic chart.
